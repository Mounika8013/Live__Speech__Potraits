...
def write_video_with_audio(audio_path, output_path, prefix='pred_'):
    ...
    out = cv2.VideoWriter(video_tmp_path, fourcc, fps, (512, 512))
...
def write_video_with_audio1(audio_path, output_path, prefix='pred_'):
    global save_root
    fps, fourcc = 60, cv2.VideoWriter_fourcc(*'mp4v')
    video_tmp_path = join(save_root, 'tmp.mp4')
    video_tmp_path1 = join(save_root, 'output.mp4')
    out = cv2.VideoWriter(video_tmp_path, fourcc, fps, (512, 512))
    for j in tqdm(range(nframe), position=0, desc='writing video'):
        img = cv2.imread(join(save_root, prefix + str(j+1) + '.jpg'))
        out.write(img)
    out.release()
    cmd = 'ffmpeg -i "'+video_tmp_path+'" -vcodec h264 "'+video_tmp_path1+'"'
    subprocess.call(cmd, shell=True) 
    cmd = 'ffmpeg -i "' + video_tmp_path1 + '" -i "' + audio_path + '" -c:v copy -c:a aac "' + output_path + '"'
    subprocess.call(cmd, shell=True) 
    os.remove(video_tmp_path)  # remove the template video
    os.remove(video_tmp_path1)  # remove the template video    

def convert(): 
    global save_root, nframe
    device = torch.device('cpu')
    with open(join('config/', 'May.yaml')) as f:
        config = yaml.load(f, Loader=yaml.Loader)
    data_root = join('data/', 'May')
    # create the results folder
    audio_name = os.path.split('./data/input/00083.wav')[1][:-4]
    save_root = join('results/', 'May', audio_name)
    ...
    # read audio
    audio, _ = librosa.load('./data/input/00083.wav', sr=sr)
    total_frames = np.int32(audio.shape[0] / sr * FPS) 
    
    
    #### 1. compute APC features   
    print('1. Computing APC features...'+str('cpu'))                    
    mel80 = utils.compute_mel_one_sequence(audio, device='cpu')
    ...
    sf.write(tmp_audio_path, tmp_audio_clip, sr, 'PCM_24')   
    
    final_path = join(save_root, audio_name + '.mp4')
    write_video_with_audio1(tmp_audio_path, final_path, 'pred_')
    #feature_maps_path = join(save_root, audio_name + '_feature_maps.avi')
    #write_video_with_audio1(tmp_audio_path, feature_maps_path, 'input_')    
...
def run():
    if os.path.exists("results/May/00083/00083.mp4") == False:
        convert()
